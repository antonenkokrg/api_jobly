...
describe("GET /jobs/:id", async function () {
  test("Gets a single a job", async function () {
    ...
    expect(response.body.jobs).toHaveProperty("id");

    expect(response.body.jobs.id).toBe(TEST_DATA.jobId);
  });
...
describe("DELETE /jobs/:id", async function () {
  ...
  test("Responds with a 500 if it cannot find the job in question", async function () {
    // delete job first
    await request(app)
      .delete(`/jobs/${TEST_DATA.jobId}`).send({ _token: TEST_DATA.userToken })
    const response = await request(app)
